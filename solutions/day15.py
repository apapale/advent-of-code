...
class PuzzleSolution(Puzzle):
    ...
    def _is_box(self, pos):
        """return bool if single site box"""
        return self._warehouse_map[pos[0]][pos[1]] == "O"

    def _is_robot(self, pos):
        """return bool if robot"""
        return self._warehouse_map[pos[0]][pos[1]] == "@"

    def _move_box(self, pos, move):
        """move box"""
        new_pos = self._sum_array(pos, move)
        move_this_box = False
    ...
    def _box_can_be_moved(self, pos, move):
    ...
    def _move_robot(self, pos, move, is_box_func, box_can_be_moved_func, move_box_func):
        """move robot"""
        new_pos = self._sum_array(pos, move)
    ...
    def _get_left_right_box_pos(self, pos):
        """given alarg box position, return left and right positions"""
        if self._is_large_box_left(pos):
            pos_l = pos
            pos_r = self._sum_array(pos, _MOVEMENTS[">"])
        else:
            # is a right side of a large box
            pos_r = pos
            pos_l = self._sum_array(pos_r, _MOVEMENTS["<"])
        return pos_l, pos_r

    def _large_box_can_be_moved_horizontal(self, new_pos, move, large_left_right_func):
        """check if large box can be moved horizontally"""
        if self._is_empty(new_pos):
            return True
        if large_left_right_func(new_pos):
            next_box = self._sum_array(new_pos, move)
            return self._large_box_can_be_moved(next_box, move)
        return None

    def _large_box_can_be_moved_vertical(self, new_pos, move):
    ...
    def _large_box_can_be_moved(self, pos, move):
        """check if large box can be moved"""
        pos_l, pos_r = self._get_left_right_box_pos(pos)
        new_pos_l = self._sum_array(pos_l, move)
        new_pos_r = self._sum_array(pos_r, move)
    ...
    def _move_large_box_horizontal(self, new_pos, move):
    ...
    def _move_large_box_vertical(self, new_pos_l, new_pos_r, move):
    ...
    def _move_large_box(self, pos, move):
        """move large box"""
        pos_l, pos_r = self._get_left_right_box_pos(pos)
        new_pos_l = self._sum_array(pos_l, move)
        new_pos_r = self._sum_array(pos_r, move)
